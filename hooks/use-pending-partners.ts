  ...
  return useQuery<AllPartnersResponse, Error>({
    ...
    queryFn: async (): Promise<AllPartnersResponse> => {
      const { data } = await api.get<ApiResponse<AllPartnersResponse>>(
        `/api/v1/admin/partner/pending`,
      )
      console.log(data, 'partnerss')
  ...
  return useQuery<PartnerDetailsResponseBody, Error>({
    ...
    queryFn: async (): Promise<PartnerDetailsResponseBody> => {
      ...
      const { data } = await api.get<ApiResponse<PartnerDetailsResponseBody>>(
        `api/v1/admin/partner/pending/details?userId=${partnerId}`, // Using userId as per your API example
      )
      if (data.responseSuccessful && data.responseBody) {
        // Add mock values for totalSimsAssigned, simsDistributed, currentSims if needed
        return {
          ...data.responseBody,
          totalSimsAssigned: data.responseBody.totalSimsAssigned || 500, // Example mock value
          simsDistributed: data.responseBody.simsDistributed || 132, // Example mock value
          currentSims: data.responseBody.currentSims || 300, // Example mock value
        }
      } else {
...
export const useApprovePartner = () => {
  ...
  return useMutation<ApiResponse<any>, Error, number>({
    mutationFn: async (partnerId: number) => {
      // Sending the status in the request body as per your API's expectation
      const { data } = await api.put<ApiResponse<any>>(`/api/v1/admin/partner/approve/${partnerId}`, {
        status: "approved",
      })
      if (data.responseSuccessful) {
        return data
      } else {
        throw new Error(data.responseMessage || "Failed to approve partner.")
      }
    },
    onSuccess: () => {
...
// Removed useDenyPartner as per your request
