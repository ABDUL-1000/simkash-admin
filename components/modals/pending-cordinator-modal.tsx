...
import { useApproveCordinator, useCordinatorDetails } from "@/hooks/use-pending-cordinator"
...
export default function PartnerDetailsModal({ partnerId, isOpen, onClose }: PartnerDetailsModalProps) {
  const { data: partnerDetails, isLoading, isError, error } = useCordinatorDetails(partnerId)
  const approveMutation = useApproveCordinator()
        ...
        <div className="flex flex-col h-full">
          ...
          {isLoading ? (
            ...
            <div className="flex-1 flex items-center justify-center text-muted-foreground">
              No cordinator selected or details not found.
            </div>
          )}
          {/* Action Buttons */}
          {partnerDetails && (
