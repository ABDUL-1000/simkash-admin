...
import { useDistributableSimBatches, useDistributeSim } from "@/hooks/use-distributeSim"
import BatchSelectWithSearch from "@/components/BatchSelect" 
...
const DistributeSimsModal: React.FC<DistributeSimsModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const [selectedBatchName, setSelectedBatchName] = useState<string | null>(null) // State to hold selected batch name
  ...
  const watchedBatchId = watch("id") // Watch the batch ID for display in subtitle

  useEffect(() => {
    if (simBatchesData && watchedBatchId) {
      const batch = simBatchesData.find((b) => b.id === watchedBatchId)
      setSelectedBatchName(batch?.batch_name || null)
    } else {
      setSelectedBatchName(null)
    }
  }, [watchedBatchId, simBatchesData])
  ...
  const onReset = () => {
    ...
    setSelectedBatchName(null) // Reset selected batch name
    reset()
    onClose()
  }
  ...
  const distributableSimBatches = simBatchesData || []
  ...
  return (
    <>
      <ReusableModal
        ...
        subTitle={selectedBatchName ? `Batch: ${selectedBatchName}` : ""}
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            ...
            <BatchSelectWithSearch
              ...
              onChange={(batchId, batchNetwork, batchName) => {
                setValue("id", batchId || 0, { shouldValidate: true })
                setSelectedBatchName(batchName) // Update local state for subtitle
                clearErrors("id") // Clear batch ID error on selection
              }}
              batches={distributableSimBatches} 
            />
          ...
          <Button
            ...
            disabled={distributeSimMutation.isPending || !phoneInfo?.verified || watchedBatchId === 0}
          >
